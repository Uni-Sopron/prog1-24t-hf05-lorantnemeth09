...
def order(guests, drinks):
    ...
    guest['balance'] -= total_price
    drink['stock'] -= quantity

    save_data(guests, "data/guests.json")
    save_data(drinks, "data/drinks.json")
    print(f"+{total_price} Ft {guest['name']} számlájára írva, egyenleg: {guest['balance']} Ft")
...
def payment(guests):
    ...
    payment_amount = int(input("Adja meg a befizetett összeget: "))
    guest['balance'] += payment_amount
    save_data(guests, "data/guests.json")
    ...
    while True:
        ...
        if option == 0:
            ...
            while True:
                ...
                print("2 - Rendelés")
                print("3 - Befizetés")
                sub_option = input("Válasszon menüpontot: ")
